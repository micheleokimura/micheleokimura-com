 ...
 *
 * Photos are plain <img> tags rather than next/image so that if a file is
 * missing (for example the first push before the timeline photos land in
 * public/), the layout stays intact and only the image icon degrades.
 */
...
function PhotoGrid({ photos }: { photos: Photo[] }) {
  ...
  return (
    <ul role="list" className={cn('grid gap-3 sm:gap-4', cols)}>
      {photos.map((p) => (
        <li key={p.src} className="group">
          <figure className="overflow-hidden rounded-2xl bg-neutral-100 ring-1 ring-neutral-200">
            <span className="relative block aspect-[4/5] w-full bg-[color-mix(in_oklab,var(--color-brand-teal)_10%,white)]">
              {/* Plain img so a missing file degrades quietly. */}
